...
const categories = [
  { id: "problem", label: "Problem Discussion", icon: Code, color: "bg-blue-500" },
  { id: "interview", label: "Interview Experience", icon: Briefcase, color: "bg-green-500" },
  { id: "career", label: "Career Advice", icon: TrendingUp, color: "bg-purple-500" },
  { id: "general", label: "General Discussion", icon: Users, color: "bg-orange-500" },
]
...
const sampleComments = [
  {
    ...
    replies: [
      {
        id: 2,
        content: "Agreed! The HashMap approach is definitely the way to go for this problem.",
        author: "codingNinja",
        authorAvatar: "/placeholder.svg?height=24&width=24",
  ...
  {
    id: 3,
    content: "Can you explain the time complexity analysis in more detail? I'm still learning about Big O notation.",
    author: "student_dev",
    authorAvatar: "/placeholder.svg?height=24&width=24",
    ...
    replies: [
      {
        ...
        replies: [
          {
            id: 5,
            content: "Thanks! That makes it much clearer. So the trade-off is space for time efficiency.",
            author: "student_dev",
            authorAvatar: "/placeholder.svg?height=24&width=24",
...
export default function GlobalDiscussion() {
    ...
    setDiscussions((prev) =>
      prev.map((discussion) => {
        if (discussion.id === discussionId) {
          ...
          return { ...discussion, comments: updateCommentVote(discussion.comments) }
        }
        return discussion
      }),
    )
          ...
          if (discussion.id === discussionId) {
            const deleteComment = (comments) => {
              return comments
                ...
                .map((comment) => ({ ...comment, replies: deleteComment(comment.replies) }))
            }
  ...
  const toggleBookmark = (discussionId) => {
    setDiscussions((prev) =>
      prev.map((discussion) =>
        discussion.id === discussionId ? { ...discussion, isBookmarked: !discussion.isBookmarked } : discussion,
      ),
    )
  }
  ...
  const handleCreatePost = () => {
    ...
    const selectedProblem = newPost.problemId ? sampleProblems.find((p) => p.id === newPost.problemId) : null
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Share experiences, discuss problems, and connect with the community</p>
        </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <Tabs
                  ...
                  onValueChange={(value) => setNewPost((prev) => ({ ...prev, contentType: value }))}
                >
                ...
                {newPost.category === "problem" && (
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setNewPost((prev) => ({ ...prev, problemId: value === "none" ? null : Number.parseInt(value) }))
                      }
                    >
                      <SelectTrigger>
                      ...
                      <SelectContent>
                        <SelectItem value="none">No specific problem</SelectItem>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setNewPost((prev) => ({ ...prev, company: value }))}
                    >
                      <SelectTrigger>
              ...
              {newPost.contentType === "text" ? (
                ...
                <div>
                  ...
                  <EditorTabs
                    ...
                    onTextChange={(value) => setNewPost((prev) => ({ ...prev, content: value }))}
                    codeValue={newPost.codeContent}
                    onCodeChange={(value) => setNewPost((prev) => ({ ...prev, codeContent: value }))}
                    codeLanguage={newPost.codeLanguage}
                    onLanguageChange={(language) => setNewPost((prev) => ({ ...prev, codeLanguage: language }))}
                  />
                </div>
              )}
      ...
      <div className="space-y-6">
        {filteredAndSortedDiscussions.map((discussion) => {
          ...
          const isCommentsExpanded = expandedComments.includes(discussion.id)
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <div className={`w-2 h-2 rounded-full ${getCategoryColor(discussion.category)}`} />
                          ...
                          {discussion.problemTitle && (
                            <>
                              ...
                              <div className="flex items-center gap-1">
                                <Badge
                                  variant={
                                    discussion.problemDifficulty === "Easy"
                                      ? "default"
                                      : discussion.problemDifficulty === "Medium"
                                        ? "secondary"
                                        : "destructive"
                                  }
                                  className="text-xs"
                                ...
                                <span className="text-sm text-muted-foreground">{discussion.problemTitle}</span>
                                <ExternalLink className="w-3 h-3 text-muted-foreground" />
                              </div>
                            </>
                          )}
                        </div>
                        <h3 className="font-semibold text-lg leading-tight mb-2">{discussion.title}</h3>
                      </div>
                      ...
                      <div className="flex items-center gap-1">
                        <Button
                          variant="ghost"
                          size="sm"
                          className={`h-8 w-8 p-0 ${discussion.isBookmarked ? "text-yellow-500" : ""}`}
                          onClick={() => toggleBookmark(discussion.id)}
                        >
                          <Star className={`w-4 h-4 ${discussion.isBookmarked ? "fill-current" : ""}`} />
                        </Button>
                        <DropdownMenu>
                ...
                <div className="ml-[75px]">
                  ...
                  {discussion.content && (
                    <div className="prose prose-sm max-w-none mb-4">
                      <div dangerouslySetInnerHTML={{ __html: formatMarkdown(discussion.content) }} />
                    </div>
                  )}

                  {/* Code Content */}
                  {discussion.contentType === "code" && discussion.codeContent && (
                    <div className="mb-4">
                ...
                <CardContent className="pt-0 border-t">
                  <div className="ml-[75px]">
                    <DiscussionComments
                      ...
                      onVoteComment={(commentId, voteType) => handleCommentVote(discussion.id, commentId, voteType)}
                      onReplyToComment={(parentId, content) => handleReplyToComment(discussion.id, parentId, content)}
                      onEditComment={(commentId, content) => handleEditComment(discussion.id, commentId, content)}
                      onDeleteComment={(commentId) => handleDeleteComment(discussion.id, commentId)}
                      onReportComment={(commentId) => handleReportComment(discussion.id, commentId)}
                    />
                  </div>
                </CardContent>
              )}
